...
st.markdown(
    ...
    <style>
        ...
        .sidebar-brand {
            ...
            color: white; padding: 16px; border-radius: 12px; text-align: center; margin-bottom: 16px;
        }
  ...
  for it in items:
    ...
    cards_html += _html(f"""
        <div class="kpi-card" style="--accent: {color};">
            <div style="display:flex; justify-content:space-between;"><span class="kpi-label">{it['label']}</span><span>{it.get('icon', '📌')}</span></div>
...
# --- HÀM RENDER DÙNG CHUNG CHO COOIS ---
...
# ================= 6. ROUTING DISPLAY ACCORDING TO MENU =================
...
elif selected_menu == "⚙️ 2. Báo Cáo Cơ Khí (3012)":
...
# --- MENU 7: BÁO CÁO NĂNG SUẤT CÁ NHÂN & QUẢN LÝ CÔNG VIỆC CON ---
elif selected_menu == "👨‍💼 7. Báo Cáo Năng Suất QC":
  render_page_header(
      "👨‍💼 Báo Cáo Năng Suất QC & Quản Lý Công Việc Con",
      "Theo dõi năng suất làm việc từng nhân sự và thiết lập danh mục công"
      " việc con theo xưởng",
      f"{tu_date.strftime('%d/%m/%Y')} → {den_date.strftime('%d/%m/%Y')}",
  )

  tab_ns1, tab_ns2 = st.tabs(
      ["📊 1. Nhật Ký & Năng Suất Cá Nhân", "⚙️ 2. Quản Lý Công Việc Con Theo Xưởng"]
  )
  ...
  with tab_ns1:
    try:
      ...
      if not df_qc_logs.empty:
        ...
        st.dataframe(df_ns_display, use_container_width=True, hide_index=True)

        buf_ns = io.BytesIO()
        with pd.ExcelWriter(buf_ns, engine="openpyxl") as writer:
          df_ns_display.to_excel(
