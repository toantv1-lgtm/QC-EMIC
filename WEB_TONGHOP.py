...
st.markdown(
    ...
    <style>
        ...
        .stApp {
            background: radial-gradient(1100px 480px at 12% -8%, #EEF1FA 0%, #F5F6F9 55%) !important;
        }
...
with tab_danh_sach:
  ...
  # SUB-TAB MỚI: BÁO CÁO NĂNG SUẤT CÁ NHÂN THEO TỪNG NGƯỜI / TỪNG NGÀY
  with tab_sub_nangsuat:
    try:
      conn = get_db_connection()
      df_qc_logs = pd.read_sql_query(
          "SELECT id, loai_qc, so_lot, ma_vt, ten_vt, ncc, tong_sl_ve, sl_kiem, sl_khong_dat, sl_dat, ket_luan, nguoi_kiem, ngay_kiem, ghi_chu FROM tb_qc_dau_vao ORDER BY ngay_kiem DESC",
          conn,
      )
      conn.close()
      ...
      if not df_qc_logs.empty:
        ...
        st.markdown(
            "#### 📊 BỘ LỌC TÍNH NĂNG SUẤT LÀM VIỆC CỦA NHÂN VIÊN QC"
        )
        col_ns1, col_flt_person, col_flt_type = st.columns([1, 1.5, 1])

        # Lấy danh sách tên người kiểm duy nhất
        ...
        with col_flt_person:
          selected_inspector = st.selectbox(
              "👤 Chọn Nhân sự QC cần tính năng suất:", list_inspectors
          )
        ...
        # Tính toán các chỉ số KPI Năng suất
        ...
        # Chuẩn bị bảng hiển thị chi tiết
        ...
        # Nút xuất Excel riêng cho báo cáo Năng suất
        buf_ns = io.BytesIO()
        ...
        st.download_button(
            ...
            file_name=f"BaoCao_NangSuat_QC_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
